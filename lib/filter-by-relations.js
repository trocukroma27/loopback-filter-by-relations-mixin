			...
			if (relation && !parentRelation) {
				...
				tasks.push((async () => {
					...
					if (relation.through) {
						...
						nestedWhere = {  [Model.app.models[name].definition.idName() || "id"]: { inq: _.pluck(relThroughFound, relation.foreignKey) } };
					} else if (relation.type == "belongsTo") {
						nestedWhere = { [relation.foreignKey]: { inq: _.pluck(relFound, "id") } };
					} else {
						nestedWhere = { [Model.app.models[name].definition.idName() || "id"]: { inq: _.pluck(relFound, relation.foreignKey) } };
					}
